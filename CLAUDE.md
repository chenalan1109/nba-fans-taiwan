...
## 最近修改紀錄（2026-05-31，需人工確認項目見文末）

| # | 變更 | 檔案 |
|---|---|---|
| 1 | 側邊欄未登入文字：`「球迷投票」登入` → `登入` | `app.py` |
| 2 | 首頁移除「球員池」KPI block | `_pages/home.py` |
| 3 | 效能優化：`get_nba_team_names` / `search_players` 加上 `@st.cache_data`（24h/1h） | `services/cached.py`, `_pages/voting.py` |
| 4 | 球迷投票頁加入「先知幣系統說明」expander | `_pages/voting.py` |
| 5 | 每日簽到系統：+150 先知幣/天，按鈕在側邊欄 | `app.py`, `services/checkin_service.py`, `database/schema.sql`, `database/db.py` |
| 6 | 新帳號初始先知幣：500 → **1000** | `services/auth_service.py` |
| 7 | 球員購買價格公式：`score × 5` → **`score × 3`**（明星球員約 100–180 幣） | `services/market_service.py` |
| 8 | 側邊欄導覽：「登入/註冊」移到第二位（首頁下面） | `app.py` |

### 需人工確認的項目
1. **簽到按鈕顯示**：登入後側邊欄應出現「每日簽到 (+150 先知幣)」按鈕；點擊後幣數更新、按鈕變為「✅ 今日已簽到」。
2. **新帳號初始幣數**：註冊後立刻到「球迷投票」確認幣數顯示 1000。
3. **球員市場價格**：進「Fantasy Team → 球員市場」，確認 Luka 約 178 幣、SGA 約 148 幣、Curry 約 117 幣（原本分別約 297/248/196）。
4. **預測系統說明**：進「球迷投票」，確認頁面頂部有「📖 先知幣系統說明」折疊區塊可展開。
5. **導覽順序**：側邊欄順序應為：首頁 → 登入/註冊 → 即時資訊牆 → 球員百科 → 球迷投票 → Fantasy Team → Matchup Debate。
6. **首頁 KPI**：首頁 KPI strip 應只剩「賽季、Matchup、資料來源」三欄，無「球員池」。
7. **切換到球迷投票速度**：第二次以後切換到球迷投票應明顯變快（NBA 隊伍名單已快取）。
8. **已有舊帳號的幣數**：舊帳號初始幣不會自動增加（只影響新帳號）；若需補發，請手動更新 DB。
